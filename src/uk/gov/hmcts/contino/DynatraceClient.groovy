      ...
      response = steps.httpRequest(
        ...
        requestBody: """{ 
          ...
          "properties": {
            ...
            "Change URL": "${steps.env.CHANGE_URL ?: 'No change URL, likely a rebuild. Refer to build URL...'}",
  ...
  def updateSyntheticTest(boolean enabled) {
    ...
    try {
      ...
      if (steps.env.TEST_URL && steps.env.DT_SYNTHETIC_TEST_ID.startsWith("HTTP") && enabled) {
        // When enabling HTTP synthetics, replace the hostname
        def hostname = steps.env.TEST_URL.replaceAll('^https?://', '').split('/')[0]
        steps.echo "Enabling HTTP synthetic - replacing hostname with TEST_URL: ${steps.env.TEST_URL}"
        ...
        for (int i = 0; i < json.script.requests.size(); i++) {
          if (json.script.requests[i].url) {
            ...
            if (oldUrl.contains("preview.platform.hmcts.net")) {
