...
class Helm {
  ...
  // Original chart metadata captured before Chart.yaml is mutated for a publish. Restored afterwards
  // so the mutation cannot leak back into the workspace and contaminate later stages.
  ...
  /**
   * Put back the original chart metadata after a preview dependency removal, so the mutated files
   * never survive past the publish that needed them.
   */
