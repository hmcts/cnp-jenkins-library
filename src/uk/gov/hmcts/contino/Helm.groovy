...
class Helm {
  ...
  private void rewriteChartDependenciesToCurrentRegistry(List<String> externalRegistries) {
    ...
    String originalChartYaml = steps.readFile(chartYamlPath)
    rewrittenChartFileBackups[chartYamlPath.toString()] = originalChartYaml

    // Chart.lock is regenerated by `helm dependency update` against the rewritten
    // repositories, so capture (or mark for removal) the lock file too.
    def chartLockPath = "${this.chartLocation}/Chart.lock"
    if (steps.fileExists(chartLockPath)) {
      rewrittenChartFileBackups[chartLockPath.toString()] = steps.readFile(chartLockPath)
    } else {
      rewriteGeneratedFiles.add(chartLockPath.toString())
    }

    String chartYaml = originalChartYaml
    externalRegistries.each { externalRegistry ->
  ...
  /**
   * Put back the original chart metadata after a sandbox dependency rewrite so the
   * mutated files never survive past the install/publish that needed them.
   */
  def restoreChartDependencyFiles() {
    ...
    steps.echo "Sandbox isolation: restored original chart dependency metadata"
