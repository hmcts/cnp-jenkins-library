...
class DotNetBuilder extends AbstractBuilder {
  ...
  def build() {
    ...
    $proc = Start-Process -NoNewWindow -PassThru -FilePath msbuild -ArgumentList "$Path /p:Configuration=Release /p:Platform='Any CP' /p:ProductVersion=1.0.0.${env.BUILD_NUMBER}"
    $proc.WaitForExit()
     '''
