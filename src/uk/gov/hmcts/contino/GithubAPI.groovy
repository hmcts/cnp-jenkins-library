...
class GithubAPI {
  ...
  def addLabels(project, issueNumber, labels) {
    ...
    def response_status = new JsonSlurper().setType(JsonParserType.LAX).parseText(JsonOutput.toJson(response))
    this.steps.echo response_status.getProperties().toString()
