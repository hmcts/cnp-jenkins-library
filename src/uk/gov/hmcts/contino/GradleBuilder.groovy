...
class GradleBuilder extends AbstractBuilder {
  ...
  def securityCheck() {
    def secrets = [
      [ secretType: 'Secret', name: 'nvd-api-key-temp-jenkins', version: '', envVariable: 'NVD_API_KEY' ]
    ]

    localSteps.withAzureKeyvault(secrets) {
      try {
        gradle("--stacktrace -DdependencyCheck.failBuild=true -Dnvd.api.key='${localSteps.env.NVD_API_KEY}' -Dnvd.api.check.validforhours=24 -Danalyzer.central.enabled=false -Danalyzer.retirejs.enabled=false -Danalyzer.ossindex.enabled=false dependencyCheckAggregate")
      } finally {
