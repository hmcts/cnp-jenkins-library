...
class HelmTest extends Specification {
  ...
  def "installOrUpgrade() on PR branch should execute without --wait flag and do manual wait"() {
    ...
    1 * steps.sh({it.containsKey('label') &&
      ...
      it.get('script').contains("Checking for initial pod creation...") &&
      it.get('script').contains("timeout 60") &&
      it.get('script').contains("-w") &&
      it.get('script').contains("grep -m1") &&
      it.get('script').contains("Running\\|Pending") &&
      it.get('script').contains("Pods detected") &&
