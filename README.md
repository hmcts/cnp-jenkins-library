...
- The opinionated pipelines (`withPipeline`, `withInfraPipeline`, `withNightlyPipeline`) place their own stages on the correct agent automatically. Build/checkout stages run on the agent for the branch's primary target environment (`aat` for `master`, `preview` for a PR build), and deployment/infrastructure stages hop onto the agent for whichever environment they're deploying to.
...
- Agent labels default to `ubuntu-<environment>`. This can be overridden per product or environment via configuration if your team needs a different agent pool — check with plat ops before doing this.
...
If `product` or `environment` can't be resolved, `withSubscription` falls back to the previous shared identity instead of scheduling onto an environment-specific agent. That's the legacy path — pass both explicitly so your pipeline gets environment-scoped scheduling and identity.
...
Nothing is tagged on push. `CHANGELOG.md` is updated from the draft's notes on every push to `master`, ahead of any tag being cut. A maintainer publishes the draft release separately, when a version is ready to cut — publishing is what creates the `vX.Y.Z` tag that pipelines can pin to.
...
`CHANGELOG.md` is maintained by the release workflow. Avoid editing generated entries by hand — the next push to `master` will regenerate them.
