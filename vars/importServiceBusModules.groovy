...
def call(String subscription) {
    ...
    String topicModuleName = sh(script: "grep '$topicIdentifier' <<< '$stateList' | awk -F '$topicIdentifier' '{print \$1}'", returnStdout: true).trim()
    String subsciptionModuleName = sh(script: "grep '$subIdentifier' <<< '$stateList' | awk -F '$subIdentifier' '{print \$1}'", returnStdout: true).trim()
