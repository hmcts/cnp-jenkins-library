...
def call(Map params) {
  ...
  if(subscriptionName.toLowerCase().contains("sbox")){
    ...
    def azureConfigDir = "/opt/jenkins/.azure-${subscription}-aks"
    def azCommand = { cmd -> return sh(script: "env AZURE_CONFIG_DIR=${azureConfigDir} az $cmd", returnStdout: true).trim() }
    azCommand "login --identity"
