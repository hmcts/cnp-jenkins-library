...
def call(DockerImage dockerImage, Map params) {
  ...
  withEnv(templateEnvVars) {
    ...
    if (!fileExists(defaultValues)) {
      ...
      WarningCollector.addPipelineWarning("deprectaed_helm_values_template", "Please provide non-templated values.yaml with helm chart", new Date().parse("dd.MM.yyyy", "27.08.2019"))
    }
