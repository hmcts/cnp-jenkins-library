...
def call(product, component, environment, planOnly, subscription) {
  ...
  log.info "Building with following input parameters: changeUrl='$changeUrl'; product='$product'; component='$component'; deploymentNamespace='$deploymentNamespace'; environment='$environment'; subscription='$subscription'; planOnly='$planOnly'"
  ...
  lock("${productName}-${environment}") {
    stage("Plan ${productName}-${environment} in ${environment}") {
      ...
      sh "terraform plan -var 'change_url=${changeUrl}' -var 'env=${environment}' -var 'name=${productName}' -var 'subscription=${subscription}' -var 'deployment_namespace=${deploymentNamespace}' -var 'product=${product}' -var 'component=${component}'" +
        (fileExists("${environment}.tfvars") ? " -var-file=${environment}.tfvars" : "")

      if (!planOnly) {
        stage("Apply ${productName}-${environment} in ${environment}") {
          sh "terraform apply -auto-approve -var 'change_url=${changeUrl}' -var 'env=${environment}' -var 'name=${productName}' -var 'subscription=${subscription}' -var 'deployment_namespace=${deploymentNamespace}' -var 'product=${product}' -var 'component=${component}'" +
            (fileExists("${environment}.tfvars") ? " -var-file=${environment}.tfvars" : "")
          parseResult = null
          try {
