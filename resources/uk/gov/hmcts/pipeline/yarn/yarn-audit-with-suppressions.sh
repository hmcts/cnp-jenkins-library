...
jq -cr 'if .advisories then .advisories | to_entries[].value else empty end' < yarn-audit-result-formatted | sort > sorted-yarn-audit-issues
...
if [[ ! -s sorted-yarn-audit-issues ]];  then
  ...
  if [ -f yarn-audit-known-issues ]; then
    ...
    jq -cr 'if .advisories then .advisories | to_entries[].value else empty end' yarn-audit-known-issues-formatted \
...
if [ ! -f yarn-audit-known-issues ]; then
  ...
  if ! jq 'if . then has("actions", "advisories", "metadata") else false end' yarn-audit-known-issues-formatted | grep -q true; then
    print_borked_known_issues
    exit 1
  fi
  ...
  jq -cr 'if .advisories then .advisories| to_entries[] | {"type": "auditAdvisory", "data": { "advisory": .value }} else empty end' yarn-audit-known-issues-formatted > yarn-audit-known-issues-result
